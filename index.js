...
app.post("/api/signup", async (req, res) => {
  ...
  await db.collection("users").doc(username).set({ username, password, level: 1, xp: 0, totalAttempts: 0, correctAnswers: 0 }, { merge: true });
  res.json({ message: "OK" });
});
...
app.get("/api/rankings", async (req, res) => {
  ...
  snapshot.forEach(doc => {
    const d = doc.data();
    users.push({ username: d.username, totalAttempts: d.totalAttempts || 0, accuracy: d.totalAttempts > 0 ? ((d.correctAnswers / d.totalAttempts) * 100).toFixed(1) : 0 });
  });
  res.json({ attemptRank: [...users].sort((a,b)=>b.totalAttempts-a.totalAttempts).slice(0,5), accuracyRank: [...users].sort((a,b)=>b.accuracy-a.accuracy).slice(0,5) });
});
...
app.post("/api/question", async (req, res) => {
  ...
  const prompt = `数学教師として、${unit}の問題を1問出せ。
  ルール: 1.数式は$で囲むLaTeX形式。バックスラッシュは2重(\\\\)にすること。2.返答は以下のJSONのみ。
